...
import org.opengts.war.report.model.DataSourceMap;
import org.opengts.war.report.model.FieldMap;
import org.opengts.war.report.model.GridMap;
import org.opengts.war.report.model.ModelMap;
import org.opengts.war.report.model.SchemaMap;
    ...
    implements Constants
{
    ...
    // new Report for Kendo UI 
    public void _writeReportNew(PrintWriter pw ,  final ReportData report)
            throws ReportException, IOException
        {
       //     HttpServletRequest request = reqState.getHttpServletRequest();
          //  int showPPBox = StringTools.parseInt(AttributeTools.getRequestAttribute(request,"showpp",""),-1);
   ...
   /*         pw.write("<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.0 Transitional//EN\">\n");
            ...
            pw.write("  <meta name='robots' content='none'/>\n");
            pw.write("  <title>" + privLabel.getPageTitle() + "</title>\n");
            WebPageAdaptor.writeCssLink(pw, reqState, "kendo.common.min.css", "css");
            WebPageAdaptor.writeCssLink(pw, reqState, "kendo.silver.min.css", "css");

            // JavaScript tools
            JavaScriptTools.writeUtilsJS(pw, request);
            JavaScriptTools.writeJSInclude(pw, JavaScriptTools.qualifyJSFileRef("jquery.min.js"), AttributeTools.GetSessionSequence(request));
            JavaScriptTools.writeJSInclude(pw, JavaScriptTools.qualifyJSFileRef("kendo.all.min.js"), AttributeTools.GetSessionSequence(request));*/
        ...
        //    pw.write("<body>\n"); 
            ...
            DataSourceMap dataosurce= new DataSourceMap();
    		SchemaMap scMap=new SchemaMap();
    		FieldMap fdMap= new FieldMap();
    		 ModelMap mdMap= new ModelMap();
		    mdMap.setFields(fdMap.getBasemap());
		    scMap.setModel(mdMap.getBasemap());
		   // dataosurce.setschema(scMap.getBasemap());
		    dataosurce.setpageSize(20);
		   // dataosurce.setData("sparsh");
		    GridMap grMap= new GridMap();
		    grMap.setFilterable(true);
		    grMap.setSortable(true);
		    grMap.setPageable(true);
		 //   grMap.setDataSource(dataosurce.getBasemap());
            ...
            DataRowTemplate rdp = report.getDataRowTemplate();
       		ReportTable table=(ReportTable)report.getReportLayout().getReportPresentation();
			...
			if (dct != null) {
				BodyColumnTemplate bct = table.getBodyColumnTemplate(dct);
				String colTitle = null;
				String colField = null;

				HeaderColumnTemplate hct = table.getHeaderColumnTemplate(dct);
				colTitle = StringTools.replace(hct.getTitle(report, rptCols[i]), "\n", " ");
            ...
            if ((data != null) && data.hasNext()) {
                int RC = 0;
                ArrayList<Object> ColMap= new ArrayList<Object>();
                for (RC = 0; RC<50; RC++) 
                {
    				   ...
    				   //  ReportColumn rptCols[] = dr.getReportColumns(); // DBDataRow
    				     ...
    				     for (int i = 0; i < rptCols.length; i++) 
    				     {
 				        	/* extract column name/arg */
   				        	...
   				        	{
   				        //		ReportTable table=(ReportTable)report.getReportLayout().getReportPresentation();
   				        		//this.rptBody.bodyRow.reportTable.getBodyColumnTemplate(dct);
   				        		BodyColumnTemplate bct = table.getBodyColumnTemplate(dct);
   				        		String fldName = bct.getFieldName(); // same as column name
   				        		Object fldVal  = dr.getDBValue(fldName, RC, rptCols[i]); // XML
   				        		rowMap.put(fldName, fldVal.toString());
   				        	//	bct.writeXML(out, level+1, RC, isTotal, tdCssClass, colSpan, fldVal, isSoapRequest);
   				        	} else {
                ...
                dataosurce.setData(ColMap);
                //this._setRecordCount(report, RC);
            }
            
            grMap.setDataSource(dataosurce.getBasemap());
            
            String json=serializer.json(grMap.getBasemap());
        //    mapProvider.writeJavaScript(pw, reqState);

            // JavaScript map points
            // see also "writeMapEvents_json", "writeMapEvents_xml"
            
            pw.write(" <div id='example'>\n");
            pw.write(" <div id='grid'></div>\n");
            JavaScriptTools.writeStartJavaScript(pw);
            JavaScriptTools.writeJS(pw, "$(document).ready(function() {  $('#grid').kendoGrid( " + json + "); });");
        ...
        throws IOException
    {
      ...
      //final String  accountID = reqState.getCurrentAccountID();
        ...
        rptFormat=ReportData.FORMAT_NEW;
        /* XML output? */
        ...
        if (rptFormat.equalsIgnoreCase(ReportData.FORMAT_NEW)) {
            /* style */
			...
			/* report */
			final ReportDisplay object= this;
			HTMLOutput HTML_CONTENT = new HTMLOutput() {
			    public void write(PrintWriter pw) throws IOException {
			        try {
			        	object._writeReportNew(pw,  reportDta);
			        } catch (ReportException re) {
